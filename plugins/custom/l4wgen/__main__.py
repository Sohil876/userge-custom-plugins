...
user_agent = {
    ...
    "Referer": "https://www.live4wap.lol",
...
async def gen_links(page_url, source_url):
    async with ClientSession(connector=TCPConnector(ssl=False)) as session:
        async with session.get(f"{page_url}", headers=user_agent) as response:
            if response.status != 200: return f"Something went wrong, aiohttp could not connect to server. Error Code: {response.status}"
            soup = BeautifulSoup(await response.text(), "html.parser")
            ...
            for idx, item in enumerate(data):
                if "1920x1080" in item:
                    msg = msg + f"* <a href={data[idx + 1]}>1920x1080 (1080p)</a>\n"
                if "1280x720" in item:
                    msg = msg + f"* <a href={data[idx + 1]}>1280x720 (720p)</a>\n"
                if "640x360" in item:
                    msg = msg + f"* <a href={data[idx + 1]}>640x360</a>\n"
                if "480x270" in item:
                    msg = msg + f"* <a href={data[idx + 1]}>480x270</a>\n"
                if "320x180" in item:
                    msg = msg + f"* <a href={data[idx + 1]}>320x180</a>\n"
            return msg
...
@userge.on_cmd(
    "l4wgen",
    about={
        "header": "Generate streaming links from live4wap channel page",
        "usage": "{tr}l4wgen https://live4wap.lol/page-watch/1/SomeChannel.html",
    },
)
async def l4wgen_(message: Message):
    ...
    url = await gen_data(page_url=arg_url)
    msg = await gen_links(page_url=url, source_url=arg_url)
